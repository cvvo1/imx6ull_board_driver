...
void lcd_drawline(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	...
	{  
		...
		lcd_drawpoint(uRow, uCol, CRlcd_dev.forecolor);/* 画点 */
		xerr += delta_x ; 
		yerr += delta_y ; 
		if(xerr > distance) 
		{ 
...
void lcd_draw_Circle(uint16_t x0,uint16_t y0,uint8_t r)
{
    ...
    {
        lcd_drawpoint(x  + mx, y  + my, CRlcd_dev.forecolor);   /*利用圆的八分对称性，只需要画出八分之一低圆弧，就可以利用对称性得到完整圆*/
        lcd_drawpoint(y  + mx, x  + my, CRlcd_dev.forecolor);
        lcd_drawpoint(-x + mx, y  + my, CRlcd_dev.forecolor);
        lcd_drawpoint(-y + mx, x  + my, CRlcd_dev.forecolor);
...
{  							  
	...
	uint8_t csize = (size / 8+ ((size % 8) ? 1 : 0)) * (size / 2);	/* 得到字体一个字符对应点阵集所占的字节数	 */	
 	num = num - ' ';  	/*得到偏移后的值（ASCII字库是从空格开始取模，所以-' '就是对应字符的字d库）  */
	for(t = 0; t < csize; t++)     /*取出点阵字节*/
	{   
		...
		for(t1 = 0; t1 < 8; t1++)     /*对字节每一个位判断是否需要点亮*/
		{			    
			if(temp & 0x80)lcd_drawpoint(x, y, CRlcd_dev.forecolor);
			else if(mode==0)lcd_drawpoint(x, y, CRlcd_dev.backcolor);
			temp <<= 1;
			y++;
			if(y >= CRlcd_dev.height) return;			/* 超区域了 */	
			if((y - y0) == size)       /*显示完一列重新复位*/
			{
				y = y0;
				x++;
				if(x >= CRlcd_dev.width) return;		/* 超区域了 */
				break;
			}
		}  	 
	}  		    	   	 	  
